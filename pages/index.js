...
export default function Home(props) {
  ...
  async function addDNA(a) {
    const { error } = await supabase.from("ca").insert({
      title: a?.dnatitle,
      description: a?.dnadesc,
      url: a?.dnaurl,
      type: "dna",
    });
    if (!error) {
  ...
  async function addImageQuizes(a) {
    const { error } = await supabase.from("ca").insert({
      image: a?.iqimage,
      title: a?.iqtitle,
      type: "iq",
  ...
  async function updateImageQuizes(a, b) {
    const { error } = await supabase
      .from("ca")
      .update({
        image: a?.image,
  ...
  async function deletePDF(a) {
    const { error } = await supabase.from("ca").delete().eq("id", a);
    if (!error) {
      getPDFs();
    } else {
    }
  }
  async function deleteDNA(a) {
    const { error } = await supabase.from("ca").delete().eq("id", a);
    if (!error) {
      getDNA();
    } else {
  ...
  async function deleteImageQuizes(a) {
    const { error } = await supabase.from("ca").delete().eq("id", a);
    if (!error) {
      getImageQuizes();
    } else {
  ...
  async function DeleteQuestion(a, b) {
    setDeleteData({
      active: true,
      id: a,
      message: `Are you sure want to delete question ${a} by ${b}?`,
    });
  }
  ...
  function removeItemAtIndex(index) {
    setCourseData((prevCourseData) => {
      ...
      if (
        Array.isArray(newCourseData.mindmaps) &&
        index >= 0 &&
        index < newCourseData.mindmaps.length
      ) {
        newCourseData.mindmaps.splice(index, 1);
      }

      return newCourseData;
    });
  }
  function toggleMCategory(isChecked, idToInsert) {
    setCourseData((prevCourseData) => {
      // Clone the previous state to avoid mutating it
      const newCourseData = { ...prevCourseData };

      // Check if mcategory array exists
      if (Array.isArray(newCourseData.mcategory)) {
        if (isChecked) {
          // Append to mcategory if isChecked is true
          newCourseData.mcategory.push(idToInsert);
        } else {
          // Remove from mcategory if isChecked is false
          const indexToRemove = newCourseData.mcategory.indexOf(idToInsert);
          if (indexToRemove !== -1) {
            newCourseData.mcategory.splice(indexToRemove, 1);
          }
        }
      } else {
        // Initialize mcategory and append if it doesn't exist
        if (isChecked) {
          newCourseData.mcategory = [idToInsert];
        }
      }
  ...
  async function addReply(a, b, c) {
    setLoading(true);
    const { data, error } = await supabase
      .from("discussion")
      .insert({
        replyto: b,
        type: props?.type == "admin" ? "teacher" : c,
        user: userData.email,
        message: a,
      })
      .select();
  ...
  function getMatchingCategories(a, b) {
    if (a == undefined) {
      return [];
    }
    const matchingObject = a.filter((item) => item.parent === b);

    if (matchingObject) {
      return matchingObject;
    } else {
      // Handle the case when no matching object is found
      return [{ title: "Not Item Found" }]; // or any other suitable default value
    }
  }
  ...
  function filterArrayByType(arr, ids) {
    if (props?.type == "admin") {
      return arr;
    } else {
  ...
  function joinAndKeepUniqueByKey(arr, key) {
    if (!Array.isArray(arr) || props?.type == "admin") {
      return null; // Return null and exit the function
    }
    // Create an array to store the values
    let values = [];

    // Loop through the array and extract values using the specified key
    arr.forEach((obj) => {
      if (Array.isArray(obj.course[key])) {
        values = values.concat(obj.course[key]);
      }
    });
          ...
          <div className="flex flex-col justify-start items-start p-8 py-8 flex-1">
            ...
            <div className="flex flex-col md:flex-row justify-start items-start  md:items-center flex-nowrap   my-3">
              ...
              <Button
                ...
                className="text-white"
                onPress={() => {
                  router.push("https://www.clattribe.com/our-courses");
                }}
              >
                Explore Courses
              </Button>
  ...
  if (props?.type != "admin" && requriedInfo == false) {
    ...
    duration: 2000,
        ...
        <div className="w-full max-w-[600px] bg-white shadow-md rounded-3xl p-6 z-10  ">
          ...
          <h2 className="text-md font-normal text-green-700">
            We need these details to keep our features working.
          </h2>
          ...
          <Input
            ...
            startContent={<div className="">{icon.whatsapp}</div>}
            placeholder="Enter your 10 Digit WhatsApp Number"
            onChange={(e) => {
              setRequiredData((res) => ({ ...res, whatsapp: e.target.value }));
            }}
          ></Input>
          <Select
            label="Your Current Centre"
            labelPlacement="outside"
            className="sf"
            placeholder="Choose your current centre"
            onChange={(e) => {
              setRequiredData((res) => ({ ...res, centre: e.target.value }));
            }}
          >
            {centres &&
              centres?.map((i, d) => {
                return (
                  <SelectItem className="sf" value={i.value} key={i.value}>
                    {i.title}
                  </SelectItem>
                );
              })}
  ...
  return (
    ...
    duration: 2000,
      ...
      {isLoggedIn == true ? (
        <IsAdminCheck type={props.type || type || "user"}>
          {" "}
              ...
              <div className=" sf h-full w-full bg-white rounded-xl relative p-2 md:p-5">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownItem isReadOnly>
                            ...
                            {mindMapData?.type == true ? (
                              <CustomEditor
                                data={
                                  mindMapData?.content?.text || "Write Here"
                                }
                                value={
                                  mindMapData?.content?.text || "Write Here"
                                }
                ...
                {slug == "games" ? (
                  <div className="w-full h-full flex flex-col justify-center align-middle items-center">
                    <GameCard
                      link="/game/pictionary"
                      GameName="Vocab Pictionary"
                      image="/grad2.webp"
                      color="white"
                    />
                  </div>
                ...
                {slug == "mathemagics" ? (
                  <div className="w-full h-full flex flex-col justify-center align-middle items-center">
                    <GameCard
                      link="/game/mathemagics"
                      GameName="Mathemagics"
                      image="/grad1.webp"
                      color="white"
                    />
                  </div>
                ...
                {slug == "resultmanage" ? <ResultManager></ResultManager> : ""}
                {slug == "print" ? <PrintManager></PrintManager> : ""}
                {slug == "submission" ? <Submissions></Submissions> : ""}
                {slug == "callsubmission" ? (
                  <CallSubmissions></CallSubmissions>
                ) : (
                  ""
                )}
                {slug == "ressubmission" ? (
                ...
                {slug == "holidays" ? <HolidayManager></HolidayManager> : ""}
                {slug == "rceditor" ? <RCManager></RCManager> : ""}
                {slug == "wotd" ? <WordManager></WordManager> : ""}
                {slug == "prv" ? (
                  <VideoGroups
                ...
                {slug == "dbts" ? (
                  <div className="w-full h-full flex flex-col items-center justify-center">
                    <h2 className="text-gray-600 text-xl">
                      Get Fast Solutions for all your Doubts on
                    </h2>
                    <h2 className="text-2xl font-bold font-sans text-primary">
                      "CLAT Tribe Community"
                    </h2>
                    <Spacer y={4}></Spacer>
                    <Button
                      ...
                      startContent={
                        ...
                        >
                          ...
                          <path
                            d="M7 9c0-1.1.9-2 2-2h6c1.1 0 2 .9 2 2v6c0 1.1-.9 2-2 2h-1l-2 2-2-2h-1c-1.1 0-2-.9-2-2V9z"
                            fill="#fff"
                          />
                        </svg>
                      }
                      as={Link}
                      href="https://chat.whatsapp.com/EIMkBl02bhr8lC36jvVCiv"
                    >
                      Open DoubtsPad
                    </Button>
                  </div>
...
Vocab by MindMaps will be available soon</h2>
                ...
                {slug == "discussion" ? (
                  <div className="h-full relative flex flex-col">
                    ...
                    {props.type == "admin" ? (
                      ...
                      <div className="p-5 border-dashed border-1 border-primary hover:border-secondary transition-all rounded-xl">
                    ...
                    <ul className="divide-y divide-gray-200 relative h-full overflow-y-auto">
                      {discussions &&
                          ...
                          props.type == "admin" ? (
                            <Dropdown>
                              <DropdownTrigger>
                                <li
                                  key={index}
                                  className=" border-1 border-grey hover:shadow-xl cursor-pointer p-4 text-left flex flex-col shadow-sm rounded-2xl hover:bg-primary transition-all my-3 "
                                >
                            ...
                            <li
                              ...
                              className="my-8"
                            >
                              <div className="border-1 border-grey hover:shadow-xl cursor-pointer p-4 text-left flex flex-col shadow-sm rounded-lg hover:bg-primary transition-all">
                                <div className="flex flex-col">
                ...
                {slug == "user" && courses != undefined ? (
                  <UserManager></UserManager>
                ) : (
                  ""
                )}
                {slug == "bulkuser" && courses != undefined ? (
                  <BulkUser></BulkUser>
                ) : (
                  ""
                )}
                ...
                {slug == "config" && <CourseManager></CourseManager>}
              </div>
            </div>
          </DefaultLayout>
        </IsAdminCheck>
