...
function Results({ result_id }) {
  ...
  async function getResults(a, b) {
    const { data, error } = await supabase
      .from("results")
      .select("*,test(title,description,id)")
      .match({ id: a, email: b });
    if (data) {
  ...
  async function getResultDetails(a) {
    const { data, error } = await supabase
      .from("scores")
      .select("*,module(title,id,subject(title))")
      .match({ result_id: a });
    if (data) {
      setResults(data);
  ...
  if (loading || loading2 || userData == undefined || resultInfo == undefined) {
    return (
      <div className="w-full h-full min-h-[100vh] bg-white flex flex-col justify-center items-center align-middle">
        <Loader></Loader>
      </div>
    );
  }
  return (
    <div className="sf bg-gray-200 w-full h-full min-h-[100vh] flex flex-col items-stretch align-middle justify-center p-5">
      <div
        className="w-full h-full absolute left-0 top-0 z-0 bg-repeat bg-[size:400px] mix-blend-multiply opacity-20"
      ...
      <div className="w-full z-10 max-w-[800px] mx-auto bg-white h-full min-h-[95vh] rounded-3xl shadow-md p-8 flex flex-col items-center align-middle justify-center">
        ...
        <div className="flex-1 w-full flex flex-col items-center align-middle justify-center">
          ...
          <h2 className="text-xl font-bold mb-4">
            Your Personalized Result for {resultInfo.test.title} is Here
          </h2>
          <Tabs
            isVertical={!isMobile}
            key={["SWOT Analysis", "Analysis"]}
            onSelectionChange={(e) => {
              setActiveButton(e);
            }}
          >
            ...
            {activeButton == 0 ? (
              <div class="relative w-72 rounded-xl overflow-hidden h-72 bg-blue-200 flex mt-4">
                <div class="absolute w-1/2 h-1/2 bg-secondary flex flex-col justify-start items-start p-3 text-black">
                  ...
                  <p className="absolute right-0 -bottom-10 w-auto h-auto flex-col flex text-[100px] opacity-20 text-white">
                    S
                  </p>
                  <div className="flex flex-col text-left text-sm justify-start max-h-[90%] overflow-y-auto">
                    {results &&
                      results
                        .filter(
                          (item) => calculateLevel(item.total, item.score) == 4,
                        )
                        .map((i, d) => {
                ...
                <div class="absolute top-0 right-0 w-1/2 h-1/2 bg-[#fc3503] flex flex-col justify-start items-start p-3 text-white">
                  ...
                  <p className="absolute right-0 -bottom-10 w-auto h-auto flex-col flex text-[100px] opacity-10 text-black">
                    W
                  </p>
                  ...
                  <div className="flex flex-col text-left text-sm justify-start max-h-[90%] overflow-y-auto">
                    {results &&
                      results
                        .filter(
                          (item) => calculateLevel(item.total, item.score) == 2,
                        )
                        .map((i, d) => {
                          return <div>{i.module.title}</div>;
                        })}
                ...
                <div class="absolute bottom-0 left-0 w-1/2 h-1/2 bg-[#9fe007] flex flex-col justify-start items-start p-3 text-white">
                  ...
                  <div className="flex flex-col text-left text-sm justify-start max-h-[90%] overflow-y-auto">
                    {results &&
                      results
                        .filter(
                          (item) => calculateLevel(item.total, item.score) == 3,
                        )
                        .map((i, d) => {
                          return <div>{i.module.title}</div>;
                        })}
                ...
                <div class="absolute right-0 bottom-0 w-1/2 h-1/2 bg-[#07c3e0] flex flex-col justify-start items-start p-3 text-black">
                  ...
                  <div className="flex flex-col text-left text-sm justify-start max-h-[90%] overflow-y-auto">
                    {results &&
                      results
                        .filter(
                          (item) => calculateLevel(item.total, item.score) == 1,
                        )
                        .map((i, d) => {
                          return <div>{i.module.title}</div>;
                        })}
            ...
            {activeButton == 1 ? (
              <div className="w-full flex flex-col bg-white  flex-wrap">
                {levels &&
                  levels.map((i, d) => {
                    return (
                      <div className="flex-0 w-full lg:flex-1 sf p-2 rounded-xl shadow-md my-2">
                        <p
                          className={`w-full p-1 text-center my-2 text-sm rounded-md ${i.color || "bg-primary"}`}
                        >
                          {i?.title}
                        </p>
                        <div className="w-full flex flex-row flex-wrap justify-center">
                          ...
                          {results &&
                            results
                              .filter((item) => item.priority == i.priority)
                              .map((z, v) => {
                                return (
                                  <div className="border-1 m-1 border-black rounded-full text-sm p-1 pl-4 mx-2">
                                    {z?.module?.title}{" "}
                                    <span className=" bg-secondary px-2 p-1 rounded-full text-black">
