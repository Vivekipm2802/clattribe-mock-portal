...
function KYCManager({ userData, role, enrolled }) {
  ...
  async function getResults() {
    ...
    console.log("KYC results data:", data); // add this
    console.log("KYC results error:", error);
    if (data) {
      setResults(data);
      setLoading2(false);
    } else {
  ...
  return (
    <div className="flex flex-col w-full h-auto">
      ...
      {tests &&
        tests?.map((i, d) => {
          return (
            <div className="w-full flex flex-row justify-between align-middle items-center rounded-md shadow-sm border-1 border-gray-100 px-4 py-3 my-1">
              <p className="font-bold text-md"> {i?.title}</p>
              <div>
                {results?.filter(
                  (item) => item.test.id == i.id && item.status == "finished",
                )?.length > 0 ? (
                  <Button
                    className="bg-blue-500 ml-2 text-white"
                    onPress={() => {
                      (router.push(
                        `/results/${results?.filter((item) => item.test.id == i.id && item.status == "finished")[0].id}`,
                      ),
                ...
                {results?.filter(
                  (item) => item.test.id == i.id && item.status == "paused",
                )?.length > 0 ? (
                  <Button
                    ...
                    onPress={() => {
                      (router.push(
                        `/kyc/${i?.uid}/?continue=${results?.filter((item) => item.test.id == i.id && item.status == "paused")[0].id}`,
                      ),
                ...
                {results?.filter((item) => item.test.id == i.id)?.length == 0 ||
