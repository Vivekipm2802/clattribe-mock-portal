...
async function getGroups(){
    const {data,error}= await supabase.from('video_groups').select('*').eq('type',type)
    if(data){
        setGroups(data)
        setLoading(false)
    }
...
async function getBatches(){
    ...
    const {data,error} = await supabase.from('batches').select('id,title')
    if(error){
...
async function deleteGroupbyId(a){
    const {data,error}= await supabase.from('video_groups').delete().eq('id',a).select()
    if(data){
...
async function toggleDemo(a,b){
    ...
    const {data,error} = await supabase.from('video_groups').update({
        demo:a
    }).eq('id',b).select()
    if(data){
        getGroups()
...
if(loading){
    ...
    return <div className="w-full overflow-hidden h-full flex-1 flex flex-col items-start justify-start">
<h2 className="text-2xl font-bold">{title}</h2>
<Spacer y={4}></Spacer>
        <div className="w-full flex overflow-y-auto flex-row flex-wrap items-stretch justify-start">
...
{(groups == undefined || groups?.length == 0) && !loading ? <><p className="w-full p-3 rounded-xl px-4 border-dashed bg-gray-50">No Content is Available here , Please check back later</p>
            ...
            {groups && groups.map((i,d)=>{
                ...
                <Spacer y={2}></Spacer>
               {type == 'lvideo' && <Select label="Batch ID" selectedKeys={[editGroupdata?.batch_id?.toString()]} placeholder="Select Batch" onSelectionChange={(e)=>{setEditGroupData(res=>({...res,batch_id:e.anchorKey}))}}>
                    {batches && batches.map((i,d)=>{
                        return <SelectItem key={i.id} value={i.id}>{i.title}</SelectItem>
                    })}
                </Select>}
                <Spacer y={2}></Spacer>
                <Button size="sm" color="primary" className="mr-auto flex-shrink-0" onPress={()=>{updateGroup(editGroupdata)}}>Update Collection</Button> 
                        </PopoverContent>
                        </Popover>
                    :''}
                    </div>
...
<Popover onOpenChange={(e)=>{e==true ? getCourses():''}}>
    <PopoverTrigger>
            ...
            <PopoverContent className="w-[400px]">
                ...
                {type == 'lvideo' && <Select label="Batch ID" selectedKeys={[groupData?.batch_id?.toString()]} placeholder="Select Batch" onSelectionChange={(e)=>{setGroupData(res=>({...res,batch_id:e.anchorKey}))}}>
                    {batches && batches.map((i,d)=>{
                        return <SelectItem key={i.id} value={i.id}>{i.title}</SelectItem>
                    })}
                </Select>}
